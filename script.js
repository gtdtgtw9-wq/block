(() => {
  ...
  let blocks = [];
  let blockAreaTop = 0, blockAreaHeight = 0;
  ...
  function layoutBlocks() {
    ...
    const bh = Math.min((blockAreaHeight - pad * (rows + 1)) / rows, 30);
  ...
  function resetBall() {
    ...
    const angle = (Math.PI / 2) + (Math.random() * 0.6 - 0.3) - Math.PI / 2; // 上向き基調
  ...
  function render() {
    ...
    if (ratio > 0) {
      if (img) {
        drawImageCover(img, 0, blockAreaTop, W, blockAreaHeight, ratio);
      } else {
        drawPlaceholder(0, blockAreaTop, W, blockAreaHeight, ratio, stageIndex);
      }
    }
  ...
  resetProgressBtn.addEventListener('click', () => {
    const ok = window.confirm('進行状況（ステージ・設定した画像）をすべてリセットします。よろしいですか？');
    if (!ok) return;
    resetSave();
    closePanel();
    resetStage(true);
